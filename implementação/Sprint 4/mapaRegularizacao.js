    ...
    tabelaInfo.on('click', 'tbody tr', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.decretoPortaria && data.decretoPortaria.length > 0) {
                    const ultimoItem = data.decretoPortaria[data.decretoPortaria.length - 1];
                    const dataDecreto = new Date(ultimoItem.data_decreto_portaria); // Supondo que a data seja em UTC.

                    if (isNaN(dataDecreto)) {
                        console.error('Data inválida:', ultimoItem.data_decreto_portaria);
                    } else {
                        // Ajusta a data para o fuso horário local
                        const dataFormatada = dataDecreto.toLocaleDateString('pt-BR'); // Formata a data para D/M/A

                        // Exibe a data formatada e o número de publicação
                        document.getElementById("info-npublicacao").innerHTML = `<ul><li>${ultimoItem.numero_publicacao} - ${dataFormatada}</li></ul>`;
                        document.getElementById("titulo-npublicacao").style.display = 'block';
                    }
                } else {
                    document.getElementById("titulo-npublicacao").style.display = 'none';
                ...
                // Ajax Modalidade
                if (data['parcelamento_info']['modalidade']) {
                    let modalidade = "<ul><li>" + data['parcelamento_info'][
                        'modalidade'
                    ] + "</li></ul>";
                    $('#info-modalidade').append($(modalidade));
                    document.getElementById("titulo-modalidade").style
                        .display = 'block';
                } else {
                    document.getElementById("titulo-modalidade").style
                        .display = 'none';
